...
MX 25.2 AHS with [niri](https://github.com/niri-wm/niri), a scrollable-tiling
Wayland compositor, in place of the Xfce desktop. **The applications are the
same as the MX 25.2 Xfce AHS edition**: only the Xfce desktop itself is replaced.

| Flavour | Defaults file | Init |
|---|---|---|
| `mxniri` | `Input/defaults-Niri-ahs` | systemd, Liquorix kernel 7.x |
| `mxniri-sysv` | `Input/defaults-Niri-ahs-sysv` | sysvinit, Liquorix kernel 6.x |
...
Mod+Alt+E switch user (login screen), Mod+Shift+Slash the full list,
Mod+Shift+E quit the session.

With LightDM autologin (always the case in the live session) quitting niri
logs straight back in: use Mod+Alt+E or the `user` button of the bar to reach
the login screen. In a virtual machine the host often keeps the Super key for
itself, so the Mod shortcuts never reach niri: enable keyboard capture in the
VM, or uncomment the `mod-key "Alt"` block at the end of `mx.kdl`.
...
**1.1** (on build-iso-mx 25.2)
- Based on the 25.2 tag: Liquorix kernel, `mx-viewer` and `nvidia-detect` as
  in the 25.2 Xfce AHS edition.
- Magnus no longer starts at login: the `Hidden=true` override of
  `magnus-autostart.desktop` from the mx theme was missing.
- No more double bar with systemd: `waybar.service`, if enabled for all
  users, is disabled; waybar is started only by `niri-mx-autostart`.
- Switch user: Mod+Alt+E and a `user` button in waybar run
  `dm-tool switch-to-greeter`.
- `zramswap.service` (a link to `/dev/null`) is kept as a link in the zip.

**1.0** first version, on build-iso-mx master.
