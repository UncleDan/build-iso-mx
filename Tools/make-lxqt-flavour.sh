...
#--- QML modules kept from the KDE list -----------------------------------
# The SDDM theme is a QML theme: without these the login screen renders
# broken. They are plain QML modules, they do not pull the Plasma shell.
...
# Conky stays enabled, exactly as in the KDE edition: the goal is a desktop
# that looks the same. It is still switchable with conky-toggle-mx, MX Tweak,
# or the Autostart checkbox in LXQt Session Settings.
...
[panel1]
# No background-color/font-color here on purpose: the LXQt theme (kde-plasma)
# provides them, so the panel matches the rest of the desktop.
alignment=-1
...
# The login screen must stay the one of the KDE edition: check that the theme
# named in sddm.conf is actually present, and say so in the build log if not.
sddm_theme=$(sed -n 's/^Current=//p' "$ROOT/etc/sddm.conf" 2>/dev/null | tail -n1)
if [ -n "$sddm_theme" ]; then
    if [ -d "$ROOT/usr/share/sddm/themes/$sddm_theme" ]; then
        echo "theme.sh: SDDM theme '$sddm_theme' present"
    else
        echo "theme.sh: WARNING - SDDM theme '$sddm_theme' is NOT installed;"
        echo "theme.sh:           the login screen will fall back to the default."
        echo "theme.sh:           Available: $(ls "$ROOT/usr/share/sddm/themes" 2>/dev/null | tr '\n' ' ')"
    fi
fi
